...
def occlude(img, num_circles, circle_size, darkness_rate):
    ...
    for _ in range(num_circles):
        ...
        for i in range(radius):
            ...
            darkness * (radius - i) / radius, darkness * (radius - i) / radius, darkness * (radius - i) / radius), 2)
...
def randResize(img, rate):
    ...
    # zoom in
    if rate > 0:
        return result[(new_height-orig_shape[0])//2:(new_height-orig_shape[0])//2 + orig_shape[0], (new_width-orig_shape[1])//2:(new_width-orig_shape[1])//2 + orig_shape[1]]
    # zoom out
    else:
        h_border = (orig_shape[1] - new_width)//2
        v_border = (orig_shape[0] - new_height)//2
        if new_width % 2 == 1:
            result = cv.copyMakeBorder(result, v_border, v_border+1, h_border+1, h_border, cv.BORDER_CONSTANT, value=(0,0,0))
        else:
...
def generateRandomScoreboard():
    ...
    scoreboard_new_tile_height = int(np.random.uniform(max(2 * new_spell_size, new_champ_size, new_item_size) + 2 * utils.SCOREBOARD_ITEM_BORDER_WIDTH + 10,
    ...
    item_coords = utils.generateItemCoordinates(new_item_size, item_left_x_offset + scoreboard_inner_left_x_offset,
    ...
    champ_coords = utils.generateChampCoordinates(new_champ_size,
