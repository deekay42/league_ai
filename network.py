...
class ChampImgNetwork(ImgNetwork):
    ...
    def __init__(self):
        super().__init__()
        self.img_size = ui_constants.CHAMP_IMG_SIZE

    def get_num_elements(self):

        return AssetManager().get_num_champ_imgs()
    ...
    def build(self):
        ...
        return regression(net, optimizer='adam', learning_rate=self.learning_rate, to_one_hot=True, n_classes=self.num_elements, shuffle_batches=True,
...
class ItemImgNetwork(ImgNetwork):
    ...
    def __init__(self):
        super().__init__()
        self.img_size = ui_constants.ITEM_IMG_SIZE

    def get_num_elements(self):
        return AssetManager().get_num_item_imgs()
...
class SelfImgNetwork(ImgNetwork):
    ...
    def __init__(self):
        super().__init__()
        self.img_size = ui_constants.SELF_IMG_SIZE
...
class PositionsNetwork(Network):
    def __init__(self):
        self.positions_game_config = \
            {
                "champs_per_team": constants.CHAMPS_PER_TEAM,
                "total_num_champs": AssetManager.AssetManager().get_num_champs(),
                "spells_per_summ": constants.SPELLS_PER_CHAMP,
                "total_num_spells": AssetManager.AssetManager().get_num_spells(),
                "rest_dim": 8
            }

        self.positions_network_config = \
            {
...
class NextItemNetwork(Network):
    ...
    def __init__(self):
        self.game_config = \
            {
                "champs_per_game": constants.CHAMPS_PER_GAME,
                "champs_per_team": constants.CHAMPS_PER_TEAM,
                "total_num_champs": AssetManager.AssetManager().get_num_champs(),
                "total_num_items": AssetManager.AssetManager().get_num_items(),
                "items_per_champ": constants.MAX_ITEMS_PER_CHAMP
            }

        self.next_network_config = \
            {
                "learning_rate": 0.00025,
                "champ_emb_dim": 6,
                "item_emb_dim": 7,
