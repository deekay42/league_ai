...
def classify_next_item(game_config, network_config):
    ...
    net = relu(
        batch_normalization(fully_connected(final_input_layer, 1024, bias=False, activation=None, regularizer="L2")))
    for i in range(5):
        net = highway(net, 1024, activation='elu', regularizer="L2", transform_dropout=0.8)
        net = dropout(net, 0.8)
